...
void Mission::step1()                    
{
    ...
    {
        sensor_msgs::Joy controlPosition;

        controlPosition.axes.push_back(xCmd);
        controlPosition.axes.push_back(yCmd);
        controlPosition.axes.push_back(zCmd);
        controlPosition.axes.push_back(yawDesiredRad);
        ctrlPosition.publish(controlPosition);
...
void Mission::step2()
{
  ...
  if(info_counter > 25)
  {
    ...
    yawrate= 0.5;
  /*!
   * @brief: if we already started breaking, keep break for 5 sample (0.1sec)
   *         and call it done, else we send normal command
   */
  ...
  if(elapsed_time > ros::Duration(0.02))
  {
    ...
    switch(square_mission.state)
    {
      ...
      case 1:
        ...
        {
          square_mission.step1();            //跳转至任务1函数完成定点靠近
        }
        else
        {
          square_mission.reset();
          ...
          square_mission.setTarget(20, 5, 0, 180);
          ROS_INFO("##### Start route %d ....", square_mission.state);
        }
        break;
      ...
      case 2:
        if(!square_mission.finished)
        {
          square_mission.step2();        //跳转之任务2函数完成环绕飞行
        }
        else
        {
          square_mission.reset();
