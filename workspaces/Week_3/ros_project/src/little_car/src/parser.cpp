...
int little_car::move_forward()
{	
	car_trans.header.frame_id = "car";
	tf::TransformListener listener;
	
	ros::Rate rate(10.0);
	while(1)
	{
		//获取car与odom坐标系之间的tf数据 
		tf::StampedTransform trans;
		try
		{
			listener.waitForTransform("car", "world", ros::Time(0), ros::Duration(3.0));
			listener.lookupTransform("car", "world", ros::Time(0), trans);
		}
	...
	//根据car与odom坐标系之间的位置关系，发布car的速度控制指令
	geometry_msgs::Twist vel_msg;
	_yaw = 4.0 * atan2(trans.getOrigin().y(), trans.getOrigin().x());
	_velocity.y = 0.5 * sqrt(pow(trans.getOrigin().x(), 2) + pow(trans.getOrigin().y(), 2));
	} 
	return 0;	
}
